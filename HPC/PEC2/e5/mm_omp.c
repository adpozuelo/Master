...
int main (int argc, char *argv[]) {
	...
	// Debug info, control OpenMP threads
	/*
	...
	*/
	...
	Z=0.0;
#pragma omp parallel for shared(a,b,Z) private(i,j) schedule(runtime)
	// We could use reduction in omp to compare results.
	for (i=0; i<N; i++) /* last matrix has been initialized */
		for (j=0; j<N; j++){
...
#pragma omp parallel for shared(a,b,c,Z) private(i,j,k,l) schedule(runtime)
	// We could use internal o ringed pragmas to compare results.
	for (i=0; i<N; i++) {
		for(j=0; j<N; j++) {    
			for (k=0; k<N; k++) {
				c[i][j] += a[i][k] * b[k][j];
				for(l=0; l<i+N+N; l++) {
					if ((l+k)>0) // avoid NaN
						c[i][j] -= ((Z*k)+l-k)/(l+k); // Division by integer is dangeours, casting to float?
				}
			}
		}
	}
...
#pragma omp parallel for shared(a,b,d) private(i,j) schedule(runtime)
	for (i=0; i<N; i++) {
		for(j=0; j<N; j++) {
			d[i][j] = a[i][j] + b[i][j];
		}
